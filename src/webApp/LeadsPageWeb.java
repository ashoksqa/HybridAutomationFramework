...
public class LeadsPageWeb extends QueriesPageWeb {// Next Extends : DesignPostersPageWeb
		...
		outerloop: for (int pager = 2; pager <= 50; pager++) {
			for (int i = 3; i <= 25; i++) {
				...
				if (mailID.equals(emailId)) {
					...
					super.assertEquals_Text(driver,testName ,"firstName", super.getText(driver,locator + 3 + "]", waitTime),
							firstName);
					super.assertEquals_Text(driver,testName ,"gender_Male_Female",
							super.getText(driver,locator + 4 + "]", waitTime), gender_Male_Female);
					super.assertEquals_Text(driver,testName ,"age", super.getText(driver,locator + 5 + "]", waitTime), age);
					super.assertEquals_Text(driver,testName ,"phoneNumber", super.getText(driver,locator + 6 + "]", waitTime),
							phoneNumber);
					super.assertEquals_Text(driver,testName ,"emailId", super.getText(driver,locator + 7 + "]", waitTime),
							emailId);
					super.assertEquals_Text(driver,testName ,"source", super.getText(driver,locator + 8 + "]", waitTime), source);
					super.assertEquals_Text(driver,testName ,"tagName", super.getText(driver,locator + 9 + "]", waitTime),
							tagName);
					break outerloop;
				}
				if(i%8==0) {
