...
public class PublishPageWeb extends HomePageWeb { // Next Extends : VisitsPageWeb
	...
	public void verifyData_PublishPageWeb(WebDriver driver, String testName,
			int draft_ionize_publishNow_Schedule_OptNumber, int news_blog_event_OptNumber, String blogTitle,
			String description, String DoctorFirtName, String date, String time) throws IOException {
		String blogType = super.pvWebApp("blogType" + news_blog_event_OptNumber);
		String countStartXpath_loc = xpath + "/html/body/app-root/app-publish/div/div/app-dashboard/div/div[3]/ul/li[";
		int count;
		int j = draft_ionize_publishNow_Schedule_OptNumber;
		if (draft_ionize_publishNow_Schedule_OptNumber == 4) {
			j = 3;
		}
		count = super.convertStrToInt(
				super.getText(driver, countStartXpath_loc + j + "]/div/span", news_blog_event_OptNumber));
		this.click_SubTabs_PublishPage(driver, draft_ionize_publishNow_Schedule_OptNumber);
			super.scrollDownFullPage_Web(driver, count / 7);
			// super.scrollUp_Web(driver,2);
		
		for (int i = count + 2; i >= 1; i--) {
			/*
			 * if (draft_ionize_publishNow_Schedule_OptNumber == 1) {
			 * super.scrollUp_Web(driver,2); i = 1; }
			 */

			String actualTitle = super.getTextOptional(driver, blogStartXpath_ListPage_loc + i + "]/div[2]", 0);
			System.out.println(testName + " == " + actualTitle + " : " + blogTitle);
			if (i % 4 == 0) {
				super.scrollUp_Web(driver, 1, 300);
			}
			if (actualTitle.equals(blogTitle)) {
				...
				if (draft_ionize_publishNow_Schedule_OptNumber == 1) {
					...
					super.assertEquals_Text(driver, testName, "BlogNextStatus",
							super.getText(driver, blogStartXpath_ListPage_loc + i + "]/div[6]/button[2]", waitTime),
							"IONIZE");
				...
				super.assertEquals_Text(driver, testName, "Doctor Name In Lists",
						super.getText(driver, blogStartXpath_ListPage_loc + i + "]/div[3]/span[1]", waitTime),
						"By " + DoctorFirtName);

				super.assertEquals_Text(driver, testName, "BlogType In Lists",
						super.getText(driver, blogStartXpath_ListPage_loc + i + "]/div[3]/span[2]", waitTime),
						"|" + blogType);

				super.assertEquals_Text(driver, testName, "DaysLeft_CenterText",
						super.removeSpaceInStr(super.getText(driver,
								blogStartXpath_ListPage_loc + i + "]/div[4]/div/div/div/span", waitTime)),
				...
				String expectedWindowTitle = "Topic : " + blogTitle + super.pvWebApp(
						"windowTitleEnd_Editor_PublishPage" + draft_ionize_publishNow_Schedule_OptNumber);
			...
			if (i == 1) {
				super.assertEquals_Text(driver, testName, "Blog Title", actualTitle, blogTitle);
				break;
			}
			 ...
			 * "Blog Title",actualTitle,blogTitle); } if
			 * (draft_ionize_publishNow_Schedule_OptNumber > 1 && i == count-10) {
			 * super.assertEquals_Text(driver, testName,
			 * "Blog Title",actualTitle,blogTitle); }
			 */
			/*else {
				if (i % 19 == 0 || i == 38 || i == 58 || i == 78 || i == 98 || i == 118 || i == 138 || i == 158
						|| i == 178) {
	...
	public void addNewBlog_PublishPageWeb(WebDriver driver, String testName, int Home_PublishPage_1_2_OptNumber,
		...
		if (Home_PublishPage_1_2_OptNumber == 1) {
			super.click_QuickStartPopup_StartBlogOptions_HomePageWeb(driver, select_NewTopic_1_2_OptNumber);
		} else if (Home_PublishPage_1_2_OptNumber == 2) {
			super.click_SideMenuTabs_HomePageWeb(driver, 2);
		...
		blogTitle_AddNewBlog = super.blogTitle_Editor_ANBPW + super.pvWebApp(opt + "_Publish");
		tagName_AddNewBlog = super.tagName_Editor_ANBPW + super.pvWebApp(opt + "_Publish");
		description_AddNewBlog = super.description_Editor_ANBPW + super.pvWebApp(opt + "_Publish");
		...
		super.setText_ByActions(driver, blogTitle_Editor_loc, blogTitle_AddNewBlog, waitTime);
		...
		if (draft_ionize_publishNow_Schedule_OptNumber == 1 || draft_ionize_publishNow_Schedule_OptNumber == 2) {
			super.click(driver, blogStatusStartXpath_Editor_loc + draft_ionize_publishNow_Schedule_OptNumber + "]",
					waitTime);
		} else {
		...
		super.kepad_Enter_ByActions_Web(driver, tagName_Editor_loc, waitTime);
		if (draft_ionize_publishNow_Schedule_OptNumber == 1 || draft_ionize_publishNow_Schedule_OptNumber == 3) {
			super.selectDateFromCalender(driver, calenderloc_Editor_loc, dateFieldXpathStartValue_loc,
					super.getTodayDate(), "owl-calendar-invalid");
		...
		super.click(driver, timeloc_Editor_loc, waitTime);
		super.click(driver, hourTxt_Editor_loc, waitTime);
		...
		if (draft_ionize_publishNow_Schedule_OptNumber == 1) {
			...
			super.assertEquals_Text(driver, testName, "CreditsUsed",
					super.getText(driver, creditsUsed_SuccessPopup_loc, waitTime),
			...
			super.assertEquals_Text(driver, testName, "DaysLeft",
					super.getText(driver, daysLeft_SuccessPopup_loc, waitTime),
			...
			super.assertContains_Text(driver, testName, "SuccessDate", "Today",
					super.getText(driver, publishDate_SuccessPopup_loc, waitTime));
			super.assertEquals_Text(driver, testName, "CreditsUsed",
					super.getText(driver, creditsUsed_SuccessPopup_loc, waitTime), "0");

		}

		super.assertEquals_Text(driver, testName, "BlogTitle",
				super.getText(driver, blogTitle_SuccessPopup_loc, waitTime), blogTitle_AddNewBlog);
